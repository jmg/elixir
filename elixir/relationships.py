...
|                    | the normal join criterion (primaryjoin) generated by   |
|                    | Elixir for the relationship. For example:              |
|                    | boston_addresses = \                                   |
...
class ManyToMany(Relationship):
    ...
    def create_tables(self):
        ...
        schema = (self.schema is not None) and schema or e1_schema
        ...
        if self.user_tablename:
            ...
            if self.inverse and source_part < target_part:
                #TODO: if self-relf, only include source_part
                tablename = "%s__%s" % (target_part, source_part)
                if options.CHECK_TABLENAME_CHANGES and \
